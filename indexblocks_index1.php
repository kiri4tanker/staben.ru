...
<?if ($arMainPageOrder && is_array($arMainPageOrder)):?>
    ...
    <?foreach ($arMainPageOrder as $key => $optionCode):?>
        ...
        <?// PARTNERS?>
        <?if ($optionCode === 'PARTNERS'):?>
            <?global $bShowPartners, $bPartnersIndexClass;?>
            <?if ($bShowPartners):?>
                <div class="drag-block container <?=$optionCode;?> <?=$bPartnersIndexClass;?>" data-class="<?=$subtype;?>_drag" data-order="<?=++$key;?>" style="--order-index: <?=$key;?>">
                    <?=CMax::ShowPageType(
                        type: 'mainpage',
                        subtype: $subtype,
                        template: $strTemplateName,
