...
$bShowPartners = ($bActiveTheme || ($arTheme['INDEX_TYPE']['SUB_PARAMS'][$indexType]['PARTNERS']['VALUE'] != 'N'));
$bPartnersIndexClass = ($arTheme['INDEX_TYPE']['SUB_PARAMS'][$indexType]['PARTNERS']['VALUE'] == 'Y' ? '' : 'hidden');
$bPartnersIndexClass .= ($arTheme['FON_PARAMS']['fon'.$indexType.'PARTNERS'] == 'Y' ? ' grey_block' : '');
...
$bShowProjects = ($bActiveTheme || ($arTheme['INDEX_TYPE']['SUB_PARAMS'][$indexType]['PROJECTS']['VALUE'] != 'N'));
$bProjectsIndexClass = ($arTheme['INDEX_TYPE']['SUB_PARAMS'][$indexType]['PROJECTS']['VALUE'] == 'Y' ? '' : 'hidden');
$bProjectsIndexClass .= ($arTheme['FON_PARAMS']['fon'.$indexType.'PROJECTS'] == 'Y' ? ' grey_block' : '');

$bShowSimple = (CMax::IsBasketPage() || CMax::IsOrderPage()) && $arTheme['SIMPLE_BASKET']['VALUE'] == 'Y';

global $arRegion;
if ($isIndex) {
    $_SESSION['ASPRO_FILTER']['arRegionLinkFront'] = $GLOBALS['arRegionLinkFront'] = ['PROPERTY_SHOW_ON_INDEX_PAGE_VALUE' => 'Y'];
}

if ($arRegion && $arTheme['USE_REGIONALITY']['DEPENDENT_PARAMS']['REGIONALITY_FILTER_ITEM']['VALUE'] == 'Y') {
    $_SESSION['ASPRO_FILTER']['arRegionLink'] = $GLOBALS['arRegionLink'] = $GLOBALS['arSideRegionLink'] = ['PROPERTY_LINK_REGION' => $arRegion['ID']];
    if ($isIndex) {
...
if ($arRegion) {
    if ($arRegion['LIST_STORES'] && !in_array('component', $arRegion['LIST_STORES'])) {
        if ($arTheme['STORES_SOURCE']['VALUE'] != 'IBLOCK') {
            $_SESSION['ASPRO_FILTER']['arRegionality'] = $GLOBALS['arRegionality'] = ['ID' => $arRegion['LIST_STORES']];
        } else {
            $_SESSION['ASPRO_FILTER']['arRegionality'] = $GLOBALS['arRegionality'] = ['PROPERTY_STORE_ID' => $arRegion['LIST_STORES']];
        }
    } else {
...
if ($isIndex) {
    $_SESSION['ASPRO_FILTER']['arrPopularSections'] = $GLOBALS['arrPopularSections'] = ['UF_POPULAR' => 1];
    $GLOBALS['arrFrontElements'] = ['PROPERTY_SHOW_ON_INDEX_PAGE_VALUE' => 'Y'];
}
