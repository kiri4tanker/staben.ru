  ...
  if ($(".bx-soa-cart-total").length) {
    ...
    if (typeof BX.Sale.OrderAjaxComponent == "object") {
      ...
      if ($("#bx-soa-auth").length && !$("#bx-soa-auth .redisigned").length) {
        ...
        // registration button in checkout for an unregistered user (are these class needed?)
      ...
      if ($(".bx-soa-cart-total-line-total").length && (asproShowLicence || asproShowOffer)) {
        ...
        let licenceBlock = $(".bx-soa .licence_block.filter");
        let offerBlock = $(".bx-soa .offer_block.filter");
        if (
          ...
          if (!licenceBlock.length && asproShowLicence && appAspro.userConsent){
            let licenseHtml = appAspro.userConsent.licenceBlock;
            $(licenseHtml).appendTo($(".license_order_wrap"));

            BX.UserConsent?.loadFromForms();
            if(BX.Sale.OrderAjaxComponent.state_licence){
              $(".bx-soa .licence_block.filter input[type=checkbox]").prop("checked", true);
            } else {
              $(".bx-soa .licence_block.filter input[type=checkbox]").prop("checked", false);
            }
          }

          if (!offerBlock.length && asproShowOffer && appAspro.userConsent){
            let offerHtml = appAspro.userConsent.offerBlock;
            $(offerHtml).appendTo($(".license_order_wrap"));
            if(BX.Sale.OrderAjaxComponent.state_offer){
              $(".bx-soa .offer_block.filter input[type=checkbox]").prop("checked", true);
            } else {
              $(".bx-soa .offer_block.filter input[type=checkbox]").prop("checked", false);
            }
          }
        ...
        if (
          ...
          BX.Sale.OrderAjaxComponent.clickOrderSaveAction = function (event) {
            if (
              ($('.bx-soa input[name="licenses_order"]').prop("checked") || !asproShowLicence) &&
            ...
            if (!$('.bx-soa input[name="licenses_order"]').prop("checked"))
