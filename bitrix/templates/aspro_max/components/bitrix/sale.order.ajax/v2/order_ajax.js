    ...
    getPersonTypeControl: function (node) {
      ...
      if (personTypesCount > 5) {
            ...
            node.appendChild(
              BX.create("DIV", {
                props: {
                  className:
                    "person-type" +
                    ("Y" == currentType.CHECKED ? " active" : "") +
                    (personTypesCount > 2 ? " hide-991" : ""),
                },
                children: [label],
              })
            );
        ...
        if (((this.regionBlockNotEmpty = !0), personTypesCount > 2)) {
          for (i in this.result.PERSON_TYPE)
            this.result.PERSON_TYPE.hasOwnProperty(i) &&
              ((currentType = this.result.PERSON_TYPE[i]),
              options.push(
                BX.create("OPTION", {
                  props: {
            ...
            serviceNode = BX.create("DIV", {
              ...
              html:
                ...
                (currentService.description && currentService.description.length
                  ? '<div class="bx-soa-service-small">' +
                    BX.util.htmlspecialchars(currentService.description) +
                    "</div>"
                  : ""),
            });
    ...
    editTotalBlock: function () {
      ...
      if (!this.result.SHOW_AUTH) {
        ...
        const createCheckboxContainer = ({ type = "", name, error, text, checked = false }) => {
          return BX.create("div", {
            props: {
              className: `${type} filter label_block onoff`,
            },
            children: [
              BX.create("label", {
                props: {
        ...
        if (!this.licensesConditions.length) {
          if (arAsproOptions["THEME"]["SHOW_LICENCE"] == "Y" && appAspro.userConsent) {
            this.licensesConditions.push(getCheckboxContainer(appAspro.userConsent.licenceBlock));
          }
          if (arAsproOptions["THEME"]["SHOW_OFFER"] == "Y" && appAspro.userConsent) {
        ...
        if (this.licensesConditions.length) {
          if (!this.totalBlockNode.querySelector(".bx-soa-cart-conditions")) {
            ...
            // if (appAspro.userConsent && appAspro.userConsent.useBitrix) {
            //   this.initUserConsent();
            // }
            ...
            checkboxes.forEach((checkbox) => {
              BX.bind(checkbox, 'change', function(e) {
                const $node = e.target;
                if ($node.checked) {
                  const oterWrap = $node.closest('.label_block');
                  if(oterWrap){
    ...
    showTotalDeliveryInfo: function () {
      ...
      if (curDelivery) {
        ...
        if (curDelivery["PERIOD_TEXT"]) {
          ...
          const removeChild = tempDiv.querySelectorAll(
            "select, input, button, iframe, a, br, hr, img, .ipol-dpd-pickup-container"
          );
          if (removeChild) {
