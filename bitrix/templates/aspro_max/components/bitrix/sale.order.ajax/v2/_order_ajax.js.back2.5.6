  ...
  BX.Sale.OrderAjaxComponent = {
    ...
    locationsCompletion: function () {
      ...
      for (i in this.locations) {
        ...
        let city_destination = this.deliveryBlockNode.querySelector('.city_destination');
        if(value){
          city_destination.classList.remove('hidden');
          this.deliveryBlockNode.querySelectorAll(".destination_value").forEach(function (node) {
            node.textContent = value;
          });
        }else{
          city_destination.classList.add('hidden');
        }
      }
    ...
    getPersonTypeControl: function (node) {
      ...
      if (personTypesCount > 5) {
        ...
        for (i in this.result.PERSON_TYPE) {
          if (this.result.PERSON_TYPE.hasOwnProperty(i)) {
            ...
            label = BX.create("LABEL", {
              ...
              events: { change: BX.proxy((function() {
                const selectType = document.querySelector(".bx-soa-person-type > select");
                null != selectType && selectType.setAttribute("name", ""), this.sendRequest()
              }), this) },
            });

            node.appendChild(
              BX.create("DIV", {
                props: { className: "person-type" + ("Y" == currentType.CHECKED ? " active" : "") + (personTypesCount > 2 ? " hide-991" : "") },
                children: [label],
              })
            );
        ...
        if (this.regionBlockNotEmpty = !0, personTypesCount > 2) {
          ...
          node.appendChild(BX.create("SELECT", {
            props: {
              name: "PERSON_TYPE",
              className: "form-control visible-991"
            },
            children: options,
            events: {
    ...
    createPaySystemItem: function (item) {
      ...
      label = BX.create("DIV", {
        props: {
          className: "bx-soa-pp-company-graf-container properties",
        },
        children: labelNodes,
      });
    ...
    getDeliveryCompactInfo: function () {
      ...
      let city_destination = this.deliveryBlockNode.querySelector('.city_destination');
      let change_city = this.deliveryBlockNode.querySelector('.change_city');
      if(BX.Sale.OrderAjaxComponent && Object.entries(BX.Sale.OrderAjaxComponent.locations).length){
        city_destination.classList.remove('hidden');
        change_city.classList.remove('hidden');
        ...
        city_destination.classList.add('hidden');
        change_city.classList.add('hidden');
      }
    ...
    editDeliveryInfo: function (deliveryNode) {
      if (!this.result.DELIVERY || !this.result.DELIVERY.length) return;
        ...
        for (i in delivery.EXTRA_SERVICES) {
          ...
          if (currentService.editControl.indexOf("this.checked") == -1) {
            ...
            serviceNode = BX.create("DIV", {
              ...
              children: [
                currentService.editControl.replace("onchange", 'id="' + serviceID[1] + '" onchange'),
                BX.create("LABEL", {
                  props: {
                    htmlFor: serviceID[1],
                  },
    ...
    createDeliveryItem: function (item) {
      ...
      label = BX.create("DIV", {
        props: {
          className:
            "bx-soa-pp-company-graf-container properties" +
      ...
      let innerChildren = [label, title, costNode, logoNode, separateNode, periodNode, calculateNode, injectNode, descNode];
    ...
    editActivePickUpBlock: function (activeNodeMode) {
      ...
      if (this.initialized.pickup) {
        ...
        var pickUpListOuter = BX.create("DIV", {
          props: { className: "bx-soa-pickup-list-outer-wrap scrollblock" },
        });
          ...
          BX.create("DIV", {
            ...
            children: [
              ...
              BX.create("DIV", {
                ...
                children: [BX.create("DIV", { props: { className: "bx-soa-pickup-l-item-desc font_xs" }, html: html })],
              }),
            ],
          }),
          BX.create("DIV", {
            ...
            children: [
              BX.create("A", {
                props: { href: "javascript:;", className: "btn btn-sm btn-transparent-border-color" },
    ...
    editActivePropsBlock: function (activeNodeMode) {
      ...
      if (this.result.USER_PROFILES || !this.result.IS_AUTHORIZED) {
        this.showFirstProps();
      } else {
    ...
    showPropItem: function (profile) {
      ...
      if (checked === "Y") {
        changeProfile = BX.create("DIV", {
          ...
          html:
            "<span class='bx-soa-change-profile font_xs'>" + BX.message("TITLE_SOA_CHANGE_INFO_SECTION") + "</span>",
    ...
    alterProperty: function (settings, propContainer) {
      ...
      if (textNode) {
        ...
        if (settings.IS_PHONE == "Y") {
          ...
          textNode.setAttribute("type2", "tel");
        }
      ...
      inputs = propContainer.querySelectorAll("input[type=text], input[type=tel]");
      for (i = 0; i < inputs.length; i++) {
    ...
    isValidForm: function () {
      ...
      if (regionErrors.length) {
        ...
        this.deliveryBlockNode.querySelector(".alert.alert-danger").style.display = "none";
        BX.cleanNode(this.deliveryBlockNode.querySelector(".alert.alert-danger"));
    ...
    bindValidationPhone: function (id, propContainer) {
      ...
      if (data && data.inputs && data.action && "object" == typeof appAspro && appAspro && appAspro.phone) {
        for (i = 0; i < $(data.inputs).length; i++) {
          if (BX.type.isElementNode(data.inputs[i]) && (appAspro.phone.init($(data.inputs[i])))) {
            BX.bind(
              data.inputs[i],
              data.action,
              BX.delegate(function () {
    ...
    validatePhone: function (input, arProperty, fieldName) {
      ...
      return "Y" == arProperty.REQUIRED && 0 == value.length && errors.push(field + " " + BX.message("SOA_REQUIRED")), "Y" == arProperty.IS_PHONE && value.length > 0 && (this.validateNormalPhone(input, errors, field), this.validateIntlPhone(input, errors, field)), errors.filter(Boolean)
    },

    validateNormalPhone: function(input, errors, field) {
      var validPhone;
      arAsproOptions.THEME.PHONE_MASK && arAsproOptions.THEME.VALIDATE_PHONE_MASK && !appAspro.phone.checkIntlPhone && (new RegExp(arAsproOptions.THEME.VALIDATE_PHONE_MASK).test($(input).val()) || errors.push(field + " " + BX.message("JS_FORMAT_ORDER")))
    },

    validateIntlPhone: function(input, errors, field) {
      if (appAspro.phone.checkIntlPhone) {
        const telInput = $(input).data("iti"),
          validPhone = telInput.isValidNumber();
        validPhone || errors.push(BX.message(appAspro.phone.errorMap[telInput.getValidationError()]))
      }
    },
    ...
    editTotalBlock: function () {
      ...
      if (!this.result.SHOW_AUTH) {
        ...
        const conditions = [];
        if (arAsproOptions["THEME"]["SHOW_LICENCE"] == "Y") {
          conditions.push(
            BX.create("span", {
              text: BX.message("CONDITIONS_TEXT_ON"),
            })
          );
          let href = arAsproOptions["SITE_DIR"] + "include/licenses_detail.php";
          let tmpUrl = BX.message("LICENSES_TEXT").match(/href="(.+?)"/);
          if (tmpUrl) {
            href = tmpUrl[1];
          }
          conditions.push(
        ...
        if (arAsproOptions["THEME"]["SHOW_OFFER"] == "Y") {
          if (arAsproOptions["THEME"]["SHOW_LICENCE"] == "Y") {
            conditions.push(
              BX.create("span", {
                text: BX.message("CONDITIONS_TEXT_AND"),
              })
            );
          }
          let href = arAsproOptions["SITE_DIR"] + "include/offer_detail.php";
          let tmpUrl = BX.message("OFFER_TEXT").match(/href="(.+?)"/);
          if (tmpUrl) {
            href = tmpUrl[1];
          }
          conditions.push(
        ...
        if (conditions.length > 0) {
          conditions.unshift(
            BX.create("span", {
              text: BX.message("CONDITIONS_TEXT"),
            })
          );
          if (!this.totalBlockNode.querySelector(".bx-soa-cart-conditions")) {
            this.totalBlockNode.querySelector(".bx-soa-total-wrapper").appendChild(
              BX.create("DIV", {
                props: {
                  className: "bx-soa-cart-conditions",
                },
                children: [
                  BX.create("div", {
                    props: {
    ...
    showTotalDeliveryInfo: function () {
      ...
      if (curDelivery) {
        ...
        if (curDelivery["PERIOD_TEXT"]) {
          ...
          const removeChild = tempDiv.querySelectorAll("select, input, button, iframe, a, br, hr, img, .ipol-dpd-pickup-container");
          if (removeChild) {
