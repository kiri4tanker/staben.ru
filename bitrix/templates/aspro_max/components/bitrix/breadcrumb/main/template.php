    ...
    for($index = 0; $index < $cnt; ++$index) {
        ...
        if ($NextSectionID) {
            ...
            if ($bShowCatalogSubsections) {
                $arSubSections = array_filter(CMax::getChainNeighbors($NextSectionID, $arItem['LINK']), fn ($arSubSection) => $arSubSection['LINK'] !== $arItem['LINK']);
            }
        }
        if($index) {
            $strReturn .= '<span class="breadcrumbs__separator">&mdash;</span>';
        }
        if($arItem['LINK'] != '' && $arItem['LINK'] != GetPagePath() && GetPagePath() != $arItem['LINK'].'index.php' || $arSubSections) {
