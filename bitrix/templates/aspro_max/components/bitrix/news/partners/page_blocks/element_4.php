		...
		if($arItems)
		{
							...
							<div class="slide-block__body">
								<ul class="menu dropdown">
									...
									<?foreach($arSections as $sId => $arSection):?>
										...
										?>
										<li class="full item<?=($bCurrent ? ' current' : '')?><?=($bCollapsed ? ' collapsed' : '')?>"><!--noindex--><a href="<?=$APPLICATION->GetCurPageParam('section_id='.$sId, $arDeleteParams)?>" class="rounded2 bordered"><span class="item_title"><?=$arSection['NAME']?></span><span class="item_count muted"><?=$arAllSections[$sId]['COUNT']?></span></a><!--/noindex--></li>
									<?endforeach;?>
			...
			<?ob_start()?>
				<?/*<div class="visible_mobile_filter">*/?>
                    ...
                    $arElementsIds = $setionIDRequest ? $arAllSections[$setionIDRequest]["ITEMS"] : $arItemsID;
                    $GLOBALS['preFilterBrand'] = ['ID' => $arElementsIds];
                    $GLOBALS['preFilterBrand'] = TSolution\Regionality::mergeSmartPreFilterWithRegionFilter($GLOBALS['preFilterBrand']);
