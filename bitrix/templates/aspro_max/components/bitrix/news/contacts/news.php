...
$arItemSelect = array('ID', 'NAME', 'IBLOCK_ID', 'DETAIL_PAGE_URL','DETAIL_TEXT','IBLOCK_SECTION_ID', 'PROPERTY_MAP', 'PROPERTY_PHONE', 'PROPERTY_SCHEDULE', 'PROPERTY_METRO', 'PROPERTY_EMAIL', 'PROPERTY_ADDRESS');
	...
	<?if($arItems):?>
		...
		foreach($arItems as $arItem)
		{
			if($arItem['PROPERTY_MAP_VALUE']){
                ...
                // use detail link?
                $showDetailLink = $arParams['SHOW_DETAIL_LINK'] === 'Y';
                $shouldHideLink = empty($arItem['DETAIL_TEXT']) && $arParams['HIDE_LINK_WHEN_NO_DETAIL'] === 'Y';
                $showLink = $showDetailLink && !$shouldHideLink;

                $address = $arItem['PROPERTY_ADDRESS_VALUE'] ? ', '.$arItem['PROPERTY_ADDRESS_VALUE'] : '';
                $itemText = $arItem['NAME'].$address;
                if ($showLink) {
                    $itemTitleHtml = '<a href="'.$arItem["DETAIL_PAGE_URL"].'" class="dark_link">'.$itemText.'</a>';
                } else {
                    $itemTitleHtml = '<div class="darken">'.$itemText.'</div>';
                }

                $html = '<div class="map_info_store">';
				$html .= '<div class="title font_mlg">'.$itemTitleHtml.'</div>';
