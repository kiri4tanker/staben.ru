...
if ($arResult['ERROR']) {
    if ($USER->IsAdmin()) {
        ?>
        <div class="alert alert-warning ordered-block">
            <?=$arResult['ERROR'];?>
        </div>
        <?
    }
...
<div class="ordered-block">
        ...
        <div class="row">
            <div class="col-sm-3">
                <div class="bx-original-item-container">
                    <?TSolution\Functions\Item::showImg($arParams, array_merge($arResult['ELEMENT'], ['SET_CONSTRUCTOR' => 'Y']), false, false, 'bx-original-item-image');?>
                        ...
                        <?foreach ($arResult['SET_ITEMS']['DEFAULT'] as $key => $arItem):?>
                            ...
                            >
                                <td class="bx-added-item-table-cell-img">
                                    <?TSolution\Functions\Item::showImg($arParams, array_merge($arItem, ['SET_CONSTRUCTOR' => 'Y']), false, false);?>
                                </td>
                                ...
                                <td class="bx-added-item-table-cell-del"><div class="bx-added-item-delete" data-role="set-delete-btn"><?=TSolution::showIconSvg('remove colored_theme_hover_text', SITE_TEMPLATE_PATH.'/images/svg/catalog/cancelfilter.svg', '', '', true, false);?></div></td>
                            </tr>
                        <?endforeach;?>
                        </tbody>
                                ...
                                foreach ($arResult['SET_ITEMS']['OTHER'] as $key => $arItem):?>
                                    ...
                                    >
                                        <div class="bx-catalog-set-item">
                                            ...
                                            <?if ($arItem['PRICE_VALUE'] || $missingPriceDisplay !== 'NOTHING'):?>
                                                <div class="bx-catalog-set-item-price">
                                                    <div class="bx-catalog-set-item-price-new font_sm">
                                                        <?if ($arItem['PRICE_VALUE'] || $missingPriceDisplay === 'PRICE'):?>
                                                            <?=$arItem['PRICE_PRINT_DISCOUNT_VALUE'];?> * <?=$arItem['BASKET_QUANTITY'];?> <?=$arItem['MEASURE']['SYMBOL_RUS'];?>
                                                        <?else:?>
                                                            <?=TSolution::getFrontParametrValue('MISSING_GOODS_PRICE_DISPLAY_TEXT_VALUE');?>
                                                        <?endif;?>
                                                    </div>
                                            ...
                                            <div class="bx-catalog-set-item-add-btn">
                                                <?if ($arItem['CAN_BUY']):?>
                                                    <button type="button" class="btn btn-default" data-role="set-add-btn">
                                                        <span class="btn-xs"><?=GetMessage('CATALOG_SET_BUTTON_ADD');?></span>
                                                    </button>
                                                <?else:?>
                                                    <?$arAddToBasket = TSolution::GetAddToBasketArray(arItem: $arItem, class_btn: 'btn-xs inline-block', arParams: $arParams);?>
                                                    <?if ($arAddToBasket['HTML']):?>
                                                        <?=$arAddToBasket['HTML'];?>
                                                    <?else:?>
                                                        <span class="bx-catalog-set-item-notavailable">
                                                            <?=GetMessage('CATALOG_SET_MESS_NOT_AVAILABLE');?>
                                                        </span>
                                                    <?endif;?>
                                                <?endif;?>
                                            </div>
        ...
        <div class="row result">
            ...
            <div class="col-sm-4" style="text-align: center;">
                ...
                <div class="bx-constructor-result-btn-container">
                    ...
                    >
                        <span class="text-uppercase"><?=GetMessage('CATALOG_SET_BUY');?></span>
                    </button>
                </div>
            </div>
        </div>
    ...
    <script type="text/javascript">
        BX.ready(() => {
            new BX.Catalog.SetConstructor(<?=CUtil::PhpToJSObject([
