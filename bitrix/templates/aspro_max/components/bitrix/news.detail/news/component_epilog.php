...
if (isset($templateData['SECTION_BNR_CONTENT']) && $templateData['SECTION_BNR_CONTENT'] == true)
{
	...
	if(isset($templateData['BNR_ON_HEAD']) && $templateData['BNR_ON_HEAD'] == true){
		...
		$GLOBALS['APPLICATION']->SetPageProperty('HEADER_LOGO', $templateData['BNR_DARK_MENU_COLOR']['VALUE'] != 'Y' ? 'light' : 'dark');
...
<?foreach($arBlockOrder as $code):?>
	<?//news?>
		...
		<?if($html && trim($html) && strpos($html, 'error') === false):?>
			...
			<div class="line-after"></div>
		<?endif;?>
	<?elseif($code == 'map' && $templateData['MAP']):?>
		<div class="detail-block ordered-block map contacts-page-map">
        <div class="ordered-block__title option-font-bold font_lg"><?=$arParams["BLOCK_MAP_NAME"]?></div>
					...
					\Aspro\Functions\CAsproMax::showBlockHtml(
						[
							...
							'ITEM' => \Aspro\Functions\CAsproMax::prepareValuesForMapInProjects([
								'ITEM' => $templateData,
								'SECTIONS' => $arResult['SECTION']['PATH']
							])
						]
					);
		...
		<div class="wraps galerys-block swipeignore  muted777 ordered-block with-title">
            ...
            <?\Aspro\Functions\CAsproMax::showBlockHtml([
                     ...
                     'PARAMS' => [
                        ...
                        'ITEMS' => $templateData['GALLERY_BIG'],
                     ]
            ]);?>
	...
	<?elseif($code == 'goods'):?>
		<?if((in_array('LINK_GOODS', $arParams['PROPERTY_CODE']) || ($arParams['SHOW_LINKED_PRODUCTS'] == 'Y' && strlen($arParams['LINKED_PRODUCTS_PROPERTY'])))
			...
			$arItemsFilter = array("IBLOCK_ID" => $catalogIBlockID, "ACTIVE" => "Y");
            $useGlobalActive = TSolution::getFrontParametrValue('USE_SECTION_GLOBAL_ACTIVE') === 'Y';

            if($useGlobalActive){
                $arItemsFilter['SECTION_GLOBAL_ACTIVE'] = 'Y';
            }
				...
				<?if($arParams['SHOW_SECTIONS_FILTER']!="N"):?>
					<div class="sections_wrap_detail">
						...
						if($arItems)
						{
                            ...
                            $countAllElements = is_array($arItems) ? count($arItems) : 0;
							...
							if($arSectionsID):?>
									...
									<?$APPLICATION->IncludeComponent(
										...
										Array(
                                            ...
                                            "ALL_ELEMENT_CNT" => $countAllElements,
										),
										false, array("HIDE_ICONS" => "Y")
									);?>
							<?endif;
