...
if(!empty($arResult['DISPLAY_PROPERTIES']['AUTHOR_REF']['DISPLAY_VALUE'])) {
    $authorId = $arResult['DISPLAY_PROPERTIES']['AUTHOR_REF']["VALUE"];
    $tableName = $arResult['DISPLAY_PROPERTIES']['AUTHOR_REF']["USER_TYPE_SETTINGS"]["TABLE_NAME"];
    $arResult['AUTHOR_REF'] = TSolution\Functions::getAuthorInfo($authorId, $tableName);
}

$arResult['DISPLAY_PROPERTIES_FORMATTED'] = CMax::PrepareItemProps($arResult['DISPLAY_PROPERTIES']);
// sort prop values by SORT
Collection::sortByColumn($arResult['DISPLAY_PROPERTIES_FORMATTED'], ['SORT' => [SORT_NUMERIC, SORT_ASC], 'NAME' => [SORT_STRING, SORT_ASC]]);
