...
$bShowAuthor = !empty($arResult['AUTHOR_REF']);
...
$templateData = array(
	...
	'MAP' => $arResult['PROPERTIES']['MAP']['VALUE'] &&
	 $arParams['SHOW_PROJECTS_MAP_DETAIL'] == 'Y' ? $arResult['PROPERTIES']['MAP']['VALUE'] : '',
	'PROPERTY_INFO_VALUE' => $arResult['PROPERTIES']['INFO']['~VALUE'],
...
<?// date active from or dates period active?>
	...
	<div class="period_wrapper_inner">
		...
		<?endif;?>
		<?if($bActiveDate || $bShowAuthor):?>
			<div class="period-block darken ncolor font_xs <?=($arResult['ACTIVE_TO'] ? 'red' : '');?> flexbox flexbox--row flexbox--gap flexbox--gap-40">
                <?if($bShowAuthor):?>
                    <div class="author flexbox flexbox--row flexbox--gap flexbox--gap-12">
                        <?$imageSrc = $arResult['AUTHOR_REF']['UF_FILE']['src'] ?? '';?>
                        <?if($imageSrc):?>
                            <img class="author__image rounded" data-src="" src="<?=$imageSrc;?>" alt="<?=$arResult['AUTHOR_REF']['UF_NAME'];?>" title="<?=$arResult['AUTHOR_REF']['UF_NAME']?>" />
                        <?endif; ?>
                        <div class="author__title"><?=$arResult['AUTHOR_REF']['UF_NAME']?></div>
                    </div>
                <?endif; ?>
                <?if($bActiveDate):?>
                    <div class="date_wrap">
						...
						<?if($bFormButton):?>
							<div class="send_message_button">
								<div class="button_wrap">
									<span class="animate-load btn btn-transparent-border-color white btn-xs" data-event="jqm" data-param-form_id="ASK_STAFF" data-autoload-staff="<?=CMax::formatJsName($arResult['NAME'])?>" data-name="ask_staff"><?=(strlen($arParams['SEND_MESSAGE_BUTTON_TEXT']) ? $arParams['SEND_MESSAGE_BUTTON_TEXT'] : Loc::getMessage('SEND_MESSAGE_BUTTON_TEXT'))?></span>
								</div>
							</div>
						<?else:?>
							<hr/>
