...
if (!empty($arResult['ITEMS'])) {
    ...
    if ($arResult['MODULES']['catalog']) {
        ...
        if ($arParams['TYPE_SKU'] === 'TYPE_1' && $arParams['DISPLAY_TYPE'] !== 'table') {
        ...
        if ($boolSKU && $featureProps = Iblock\Model\PropertyFeature::getListPageShowPropertyCodes($arSKU['IBLOCK_ID'], ['CODE' => 'Y'])) {
            $arParams['OFFERS_PROPERTY_CODE'] = $featureProps;
        }
        if ($boolSKU && !empty($arParams['OFFER_TREE_PROPS']) && $arParams['TYPE_SKU'] == 'TYPE_1' && $arParams['DISPLAY_TYPE'] != 'table') {
    ...
    foreach ($arResult['ITEMS'] as $key => $arItem) {
        ...
        if (!isset($arItem['CATALOG_SUBSCRIPTION']) || $arItem['CATALOG_SUBSCRIPTION'] != 'Y') {
        ...
        if ($arParams['ADD_PICT_PROP'] && isset($arItem['PROPERTIES'][$arParams['ADD_PICT_PROP']]) && $arItem['PROPERTIES'][$arParams['ADD_PICT_PROP']] && $arItem['PROPERTIES'][$arParams['ADD_PICT_PROP']]['VALUE']) {
            if (!is_array($arItem['PROPERTIES'][$arParams['ADD_PICT_PROP']]['VALUE'])) {
                $arItem['PROPERTIES'][$arParams['ADD_PICT_PROP']]['VALUE'] = [$arItem['PROPERTIES'][$arParams['ADD_PICT_PROP']]['VALUE']];
                $arItem['PROPERTIES'][$arParams['ADD_PICT_PROP']]['DESCRIPTION'] = [$arItem['PROPERTIES'][$arParams['ADD_PICT_PROP']]['DESCRIPTION']];
            }
        ...
        if ($arItem['PREVIEW_PICTURE']) {
            // method CMax::getSliderForItemExt using DETAIL_PICTURE
            $arItem['DETAIL_PICTURE'] = $arItem['PREVIEW_PICTURE'];

            if ($arItem['IPROPERTY_VALUES']) {
                $arItem['IPROPERTY_VALUES']['ELEMENT_DETAIL_PICTURE_FILE_ALT'] = $arItem['IPROPERTY_VALUES']['ELEMENT_PREVIEW_PICTURE_FILE_ALT'];
                $arItem['IPROPERTY_VALUES']['ELEMENT_DETAIL_PICTURE_FILE_TITLE'] = $arItem['IPROPERTY_VALUES']['ELEMENT_PREVIEW_PICTURE_FILE_TITLE'];
            }
        }

        $arItem['ALT_TITLE_GET'] = $arParams['ALT_TITLE_GET'];
        $arItem['GALLERY'] = CMax::getSliderForItemExt($arItem, $arParams['ADD_PICT_PROP'], $arParams['ADD_DETAIL_TO_SLIDER'] == 'Y');
        ...
        if ($arItem['CATALOG'] && isset($arItem['OFFERS']) && !empty($arItem['OFFERS'])) {
            if ($arParams['TYPE_SKU'] == 'TYPE_1' && $arParams['DISPLAY_TYPE'] != 'table') {
                ...
                $arPropSKU = $arItem['OFFERS_PROPS_JS'] = [];
                ...
                foreach ($arSKUPropIDs as $propkey => $strOneCode) {
                    ...
                    if ($arPropSKU[$strOneCode]) {
                        Collection::sortByColumn($arPropSKU[$strOneCode], ['SORT' => [SORT_NUMERIC, SORT_ASC], 'NAME' => [SORT_NUMERIC, SORT_ASC]]); // sort sku prop values
                        $arItem['OFFERS_PROPS_JS'][$strOneCode] = [
                ...
                if ($arParams['SET_SKU_TITLE'] === 'Y') {
                    if (isset($arItem['OFFERS_SELECTED'])) {
                        $ipropValues = new Iblock\InheritedProperty\ElementValues($arItem['OFFERS'][$arItem['OFFERS_SELECTED']]['IBLOCK_ID'], $arItem['OFFERS'][$arItem['OFFERS_SELECTED']]['ID']);
        ...
        if (!empty($arItem['DISPLAY_PROPERTIES'])) {
            foreach ($arItem['DISPLAY_PROPERTIES'] as $propKey => $arDispProp) {
                ...
                if ($arDispProp['PROPERTY_TYPE'] == 'F' || $propKey == 'CML2_ARTICLE' || $arDispProp['CODE'] == $arParams['STIKERS_PROP']) {
                    unset($arItem['DISPLAY_PROPERTIES'][$propKey]);
                }
            }
            $arItem['DISPLAY_PROPERTIES'] = TSolution::PrepareItemProps($arItem['DISPLAY_PROPERTIES']);
            TSolution\LinkableProperty::resolve($arItem['DISPLAY_PROPERTIES'], $arItem['IBLOCK_ID'], $arItem['IBLOCK_SECTION_ID']);
        }
