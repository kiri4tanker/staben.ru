...
foreach ($this->basketItems as $row)
{
    ...
    if($row['SKU_DATA']) {
        ...
        // $rowData['PRODUCT_ID'] = $parentItemIDs['ID'];
        // $rowData['IBLOCK_ID'] = $parentItemIDs['IBLOCK_ID'];
        $rowData['DETAIL_PAGE_URL'] = CIBlockElement::GetByID($row['PRODUCT_ID'])->GetNext()['DETAIL_PAGE_URL'];
    }
            ...
            elseif (!empty($row[$value['id']]))
            {
                ...
                if (strpos($value['id'], 'PROPERTY_') !== false) {
                    if ($propCode = str_replace(['PROPERTY_', '_VALUE'], '', $value['id'])) {
                        ...
                        if ($arProp) {
                            if ($arProp['USER_TYPE'] === 'SAsproMaxTextWithLink') {
                                continue;
                            }
                        }
                    }
