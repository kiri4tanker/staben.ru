(function () {
  ...
  BX.Sale.BasketComponent = {
    ...
    init: function (parameters) {
      ...
      if (this.getCacheNode(this.ids.basketRoot)) {
    ...
    getCacheNode: function (id) {
      if (typeof this.nodes === "undefined") return null;
    ...
    showSimilarCount: function (state) {
      ...
      if (BX.type.isDomNode(itemCountNode)) {
        if (state) {
          itemCountNode.innerHTML =
            this.sortedItems.length + " " + this.getGoodsMessage(this.result.BASKET_ITEMS_COUNT, "SBB_SIMILAR_ITEM");
          itemCountNode.style.display = "";
        } else {
        ...
        if (this.shownItems.length && sortIndex >= 0) {
          if (sortIndex < BX.util.array_search(this.shownItems[0], this.sortedItems)) {
            ...
            if (BX.type.isDomNode(BX(this.ids.itemServices + this.shownItems[this.shownItems.length - 1]))) {
              ...
              BX(this.ids.item + this.shownItems[this.shownItems.length - 1]).insertAdjacentHTML(
                "afterend",
                basketServicesHtml
              );
            }
