...
function addShareBasket(parent) {
    if(addShareBasket && arAsproOptions["THEME"]["SHOW_SHARE_BASKET"] === "Y"){
        if (arAsproOptions.PRICES.MIN_PRICE && arAsproOptions.PRICES.MIN_PRICE > BX.Sale.BasketComponent.result?.allSum){
          BX.remove(document.querySelector(".basket-checkout-block-share"));
