    ...
    <div class="reviews-gallery-block" >
        ...
        <div class="owl-carousel owl-theme owl-bg-nav short-nav appear-block" data-plugin-options='{"smartSpeed":1000, "dots": false, "nav": true, "loop": false, "index": true, "margin": 10, "responsive": {"0":{"items": 4},"601":{"items": 6, "autoWidth": false, "lightDrag": false},"768":{"items": 8},"992":{"items": 8}, "1200":{"items": 10}}}'>
            <?foreach ($arResult['IMAGES'] as $i => $arPhoto):?>
                ...
                $arImageFileSmall = CFile::ResizeImageGet($arPhoto['FILE_ID'], ['width' => 180, 'height' => 180], BX_RESIZE_IMAGE_PROPORTIONAL_ALT)['src'];
                ?>
                <div class="item">
...
<div class="blog-comments" id="blg-comment-<?=$arParams['ID'];?>">
    ...
    <?else:?>
                    ...
                    <div class="blog-comment-form rounded3 bordered">
                        <form enctype="multipart/form-data" method="POST" name="form_comment" id="form_comment" action="<?=$ajaxPath;?>">
                            ...
                            <?if (isset($_REQUEST['XML_ID'])):?>
                                <input type="hidden" name="XML_ID" value="<?=htmlspecialcharsbx($_REQUEST['XML_ID']);?>">
                            <?endif;?>
                            ...
                            <div class="form blog-comment-fields">
                                            ...
                                            <div class="votes_block nstar big with-text">
                                                <div class="ratings">
                                                    <div class="inner_rating">
                                                        <?for ($i = 1; $i <= 5; ++$i):?>
                                                            <div class="item-rating" data-message="<?=GetMessage('RATING_MESSAGE_'.$i);?>"><?=TSolution::showIconSvg('star', SITE_TEMPLATE_PATH.'/images/svg/star_lg.svg');?></div>
                                                        <?endfor;?>
                                                    </div>
                                                </div>
                                ...
                                <?if ($arResult['COMMENT_PROPERTIES']['SHOW'] == 'Y'):?>
                                    ...
                                    $eventHandlerID = AddEventHandler('main', 'system.field.edit.file', ['CBlogTools', 'blogUFfileEdit']);
                                    ?>
        ...
        <?php
        $prevTab = 0;
        function ShowComment($comment, $tabCount = 0, $tabSize = 2.5, $canModerate = false, $User = [], $use_captcha = false, $bCanUserComment = false, $errorComment = false, $arParams = [])
        {
            ...
            if ($comment['SHOW_AS_HIDDEN'] == 'Y' || $comment['PUBLISH_STATUS'] == BLOG_PUBLISH_STATUS_PUBLISH || $comment['SHOW_SCREENNED'] == 'Y' || $comment['ID'] == 'preview') {
                ...
                <?if (
                        ...
                        <div class="blog-comment-cont-white">
                            <div class="blog-comment-info">
                                <?if ($tabCount > 0 || $comment['PARENT_ID']):?>
                                    <?=TSolution::showIconSvg('arrow_answer', SITE_TEMPLATE_PATH.'/images/svg/arrow_answer.svg');?>
                                <?endif;?>
                                ...
                                <div class="blog-info__rating">
                                    <div class="votes_block nstar big with-text">
                                        <div class="ratings">
                                            <div class="inner_rating">
                                                <?for ($i = 1; $i <= 5; ++$i):?>
                                                    <div class="item-rating <?=$i <= $comment['UF_ASPRO_COM_RATING'] ? 'filed' : '';?>"><?=TSolution::showIconSvg('star', SITE_TEMPLATE_PATH.'/images/svg/star.svg');?></div>
                                                <?endfor;?>
                                            </div>
                                        </div>
                                    </div>
                            ...
                            <?if (isset($comment['UF_ASPRO_COM_APPROVE']) && $comment['UF_ASPRO_COM_APPROVE']):?>
                                <div class="blog-comment-approve alert alert-success alert-success--with-icon font_sxs">
                                    <?=TSolution::showSpriteIconSvg(SITE_TEMPLATE_PATH.'/images/svg/content_icons_sprite.svg#icon-checkmark', 'svg-checkmark', [
                                        'HEIGHT' => 7,
                                        'WIDTH' => 10,
                                    ]);?>
                            ...
                            <div class="blog-comment-content">
                                        ...
                                        <div class="reviews-gallery-block" >
                                            <div class="owl-carousel owl-theme owl-bg-nav short-nav appear-block" data-plugin-options='{"smartSpeed":1000, "dots": false, "nav": true, "loop": false, "index": true, "margin": 10, "responsive": {"0":{"items": 2},"601":{"items": 3, "autoWidth": false, "lightDrag": false},"768":{"items": 4},"992":{"items": 4}, "1200":{"items": 5}}}'>
                                                <?foreach ($comment['IMAGES'] as $arImg):?>
                                                    <?php
                                                    $arImageFileSmallDetail = CFile::ResizeImageGet($arImg['FILE_ID'], ['width' => 360, 'height' => 360], BX_RESIZE_IMAGE_PROPORTIONAL_ALT)['src'];
                                ...
                                <?if ($comment['COMMENT_PROPERTIES']['SHOW'] == 'Y'):?>
                                    <?$eventHandlerID = AddEventHandler('main', 'system.field.view.file', ['CBlogTools', 'blogUFfileShow']);?>
                                    <div>
                                        <?php
                                ...
                                <div class="blog-comment-meta">
                                    ...
                                    <?if (strlen($comment['urlToDelete']) > 0):?>
                                        <?$targetURL = $comment['urlToDelete'].'&'.bitrix_sessid_get().'&IBLOCK_ID='.htmlspecialcharsbx($request['IBLOCK_ID']).'&ELEMENT_ID='.htmlspecialcharsbx($request['ELEMENT_ID']);?>
                                    ...
                                    <?if (strlen($comment['urlToSpam']) > 0):?>
                                        <span class="blog-comment-delete blog-comment-action blog-comment-spam color_222">
                                            <a href="<?=$comment['urlToSpam'];?>"
                                                class="blog-comment-action__link muted"
                                                title="<?=GetMessage('BPC_MES_SPAM_TITLE');?>"
                                            ><?=GetMessage('BPC_MES_SPAM');?></a>
                                        </span>
                                    <?endif;?>
                    ...
                    if (
                        ...
                        && (
                            strlen($_POST['preview']) > 0
                            && $_POST['show_preview'] != 'N'
                        )
                        && (
                            intval($_POST['parentId']) > 0
                            || intval($_POST['edit_id']) > 0
                        )
                        && (
                            (
                                intval($_POST['parentId']) == $comment['ID']
                                && intval($_POST['edit_id']) <= 0
                            )
                            || (
                                intval($_POST['edit_id']) > 0
                                && intval($_POST['edit_id']) == $comment['ID']
                                && $comment['CAN_EDIT'] == 'Y'
                            )
                        )
                    ) {
                        ...
                        ShowComment($commentPreview, (intval($_POST['edit_id']) == $comment['ID'] && $comment['CAN_EDIT'] == 'Y') ? $level : ($level + 1), 2.5, false, [], false, false, false, $arParams);
                    }
            ...
            if ($arResult['CanUserComment']) {
                ...
                <div class="blog-add-comment">
                    <a class="btn btn-lg btn-transparent-border-color white"
                        href="javascript:void(0)"
                    ><?=GetMessage('B_B_MS_ADD_COMMENT');?></a>
                </div>
        ...
        if ($arResult['is_ajax_post'] != 'Y' && $arResult['NEED_NAV'] == 'Y') {
            for ($i = 1; $i <= $arResult['PAGE_COUNT']; ++$i) {
                ...
                ?>
                <div id="blog-comment-page-<?=$i;?>"<?=$arResult['PAGE'] != $i ? 'style="display:none;"' : '';?>>
            ...
            if (
                !$arResult['CommentsResult'][0]
                && !$arResult['ajax_comment']
                && !strlen($arResult['COMMENT_ERROR'])
            ) {
                ?>
