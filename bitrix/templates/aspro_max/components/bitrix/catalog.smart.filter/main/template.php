...
if ($arResult["ITEMS"]) {?>
    ...
    <?$bActiveFilter = $arParams["SEF_RULE_FILTER"] ? \Aspro\Functions\CAsproMax::checkActiveFilterPage($arParams["SEF_RULE_FILTER"]) : strpos($APPLICATION->GetCurPageParam(), 'set_filter') !== false;?>
                ...
                <div class="bx_filter_section bordered rounded3">
                    <form name="<?echo $arResult["FILTER_NAME"]."_form"?>" action="<?echo $arResult["FORM_ACTION"]?>" method="get" class="smartfilter">
                        <div class="bx_filter_parameters">
                            ...
                            <?endforeach;
                            $isFilter=false;
                            $numVisiblePropValues = TSolution\Utils::getNumVisibleCountFilter();
                            ...
                            foreach ($arResult["ITEMS"] as $key => $arItem) {
                                if (isset($arItem["ASPRO_FILTER_SORT"]) && $arItem["VALUES"]) {
                                    ...
                                    ?>
                                    <div class="bx_filter_parameters_box bx_sort_filter visible-xs <?=$class;?>" data-expanded="<?=($arItem["DISPLAY_EXPANDED"] ? $arItem["DISPLAY_EXPANDED"] : "N");?>" data-prop_code="<?=strtolower($arItem["CODE"]);?>" data-property_id="<?=$arItem["ID"]?>">
                            ...
                            foreach ($arResult["ITEMS"] as $key=>$arItem)
                            {
                                ...
                                <div class="bx_filter_parameters_box <?=$class;?> <?=(isset($arItem['PROPERTY_SET']) && $arItem['PROPERTY_SET'] == 'Y' ? ' set' : '');?>" data-expanded="<?=($arItem["DISPLAY_EXPANDED"] ? $arItem["DISPLAY_EXPANDED"] : "N");?>" data-prop_code=<?=strtolower($arItem["CODE"]);?> data-property_id="<?=$arItem["ID"]?>" data-property_name="<?=$arItem["NAME"]?>">
                                    ...
                                    <div class="bx_filter_block <?=(!$arItem["IS_PROP_INLINE"] && $arItem["DISPLAY_TYPE"] != 'A' ? '' : '');?> <?=(!$arItem["IS_PROP_INLINE"] && $arItem["PROPERTY_TYPE"]!="N" && ($arItem["DISPLAY_TYPE"] != "P" && $arItem["DISPLAY_TYPE"] != "R") ? "limited_block" : "");?>" <?=$style;?>>
                                        <?
                                        if( TSolution\Utils::checkShowSearchFilter($arItem, $arParams) ):?>
                                            <?$templateData['USE_FILTER_SEARCH'] = true;?>
                                            <?TSolution\Functions::showBlockHtml([
                                                'FILE' => '/catalog/filter/search.php',
                                                'PARAMS' => [
                                                    'ITEM' => $arItem,
                                                ]
                                            ])?>
                                        <?endif;?>
                                        <div class="bx_filter_parameters_box_container <?=($arItem["DISPLAY_TYPE"]=="G" ? "pict_block" : "");?> <?=(!$arItem["IS_PROP_INLINE"] && $arItem["PROPERTY_TYPE"]!="N" && ($arItem["DISPLAY_TYPE"] != "P" && $arItem["DISPLAY_TYPE"] != "R") ? "scrollblock scrollblock--ob-auto" : "");?>">
                                        <?
                                                        ...
                                                        <div class="pict filter-value-wrap">
                                                            ...
                                                            <label for="<?=$ar["CONTROL_ID"]?>" data-role="label_<?=$ar["CONTROL_ID"]?>" class="bx_filter_param_label nab dib<?=$class?>" onclick="smartFilter.keyup(BX('<?=CUtil::JSEscape($ar["CONTROL_ID"])?>')); BX.toggleClass(this, 'active');">
                                                                <?/*<span class="bx_filter_param_btn bx_color_sl" title="<?=$ar["VALUE"]?>">*/?>
                                                                    ...
                                                                    <span class="bx_filter_btn_color_icon filter-value-text" title="<?=$ar["VALUE"]?>" style="background-image:url('<?=$ar["FILE"]["SRC"]?>');"></span>
                                                                    <?endif?>
                                                                <?/*</span>*/?>
                                                            </label>
                                                        </div>
                                                    ...
                                                    <?if ($ar["VALUE"]) {?>
                                                        ...
                                                        <label for="<?=$ar["CONTROL_ID"]?>" data-role="label_<?=$ar["CONTROL_ID"]?>" class="bx_filter_param_label<?=$class?> pal nab" onclick="smartFilter.keyup(BX('<?=CUtil::JSEscape($ar["CONTROL_ID"])?>')); BX.toggleClass(this, 'active');">
                                                            <?/*<span class="bx_filter_param_btn bx_color_sl" title="<?=$ar["VALUE"]?>">*/?>
                                                                ...
                                                                <?endif?>
                                                            <?/*</span>*/?>
                                                                <span class="bx_filter_param_text filter-value-text" title="<?=$ar["VALUE"];?>"><?=$ar["VALUE"];?><?
                                                    ...
                                                    <div class="filter label_block radio filter-value-wrap">
                                                        ...
                                                        <label data-role="label_<?=$ar["CONTROL_ID"]?>" class="bx_filter_param_label <?=$class;?>" for="<? echo $ar["CONTROL_ID"] ?>">
                                                            <span class="bx_filter_input_checkbox">
                                                                ...
                                                                <span class="bx_filter_param_text1 filter-value-text" title="<?=$ar["VALUE"];?>"><?=$ar["VALUE"];?><?
                                                                if ($arParams["DISPLAY_ELEMENT_COUNT"] !== "N" && isset($ar["ELEMENT_COUNT"])):
                                                                    ?> (<span data-role="count_<?=$ar["CONTROL_ID"]?>"><? echo $ar["ELEMENT_COUNT"]; ?></span>)<?
                                                                endif;?></span>
                                                            </span>
