    ...
    foreach($productSlider as $i => $arImage) {
        $productSlider[$i] = array_merge(
            $arImage, [
                ...
                'SMALL' => CFile::ResizeImageGet($arImage['ID'], ['width' => $arSizePict[0], 'height' => $arSizePict[1]], BX_RESIZE_IMAGE_PROPORTIONAL, true, []),
        ...
        if($arOffer['MORE_PHOTO']) {
            foreach($arOffer['MORE_PHOTO'] as $i => $arImage) {
                if($arImage['ID']) {
                    ...
                    $arOffer['MORE_PHOTO'][$i]['SMALL'] = CFile::ResizeImageGet($arImage['ID'], ['width' => $arSizePict[0], 'height' => $arSizePict[1]], BX_RESIZE_IMAGE_PROPORTIONAL, true, []);
...
if(($arResult['DISPLAY_PROPERTIES']['BRAND']['VALUE'] ?? false) && $arResult['PROPERTIES']['BRAND']['LINK_IBLOCK_ID']) {
    ...
    if($arBrand) {
        if($arParams['SHOW_BRAND_PICTURE'] == 'Y' && ($arBrand['PREVIEW_PICTURE'] || $arBrand['DETAIL_PICTURE'])) {
            $arBrand['IMAGE'] = CFile::ResizeImageGet($arBrand['PREVIEW_PICTURE'] ? $arBrand['PREVIEW_PICTURE'] : $arBrand['DETAIL_PICTURE'], ['width' => 200, 'height' => 40], BX_RESIZE_IMAGE_PROPORTIONAL_ALT, true);
        }
    }
}
...
if($arResult['SECTION']) {
    $arSection = CMaxCache::CIBlockSection_GetList(['CACHE' => ['MULTI' => 'N', 'TAG' => CMaxCache::GetIBlockCacheTag($arParams['IBLOCK_ID'])]], ['GLOBAL_ACTIVE' => 'Y', 'ID' => $arResult['SECTION']['ID'], 'IBLOCK_ID' => $arParams['IBLOCK_ID']], false, ['ID', 'IBLOCK_ID', 'IBLOCK_SECTION_ID', 'LEFT_MARGIN', 'RIGHT_MARGIN', 'NAME', 'UF_TABLE_SIZES']);
    ...
    if(!$tmpTableSizes) {
        if($arResult['SECTION']['DEPTH_LEVEL'] > 2) {
            $arSectionParent = CMaxCache::CIBlockSection_GetList(['CACHE' => ['MULTI' => 'N', 'TAG' => CMaxCache::GetIBlockCacheTag($arParams['IBLOCK_ID'])]], ['GLOBAL_ACTIVE' => 'Y', 'ID' => $arSection['IBLOCK_SECTION_ID'], 'IBLOCK_ID' => $arParams['IBLOCK_ID']], false, ['ID', 'IBLOCK_ID', 'NAME', 'UF_TABLE_SIZES']);
            if($arSectionParent['UF_TABLE_SIZES']) {
                $tmpTableSizes = $arSectionParent['UF_TABLE_SIZES'];
            }

            if(!$tmpTableSizes) {
                $sectionRoot = CMaxCache::CIBlockSection_GetList(['CACHE' => ['MULTI' => 'N', 'TAG' => CMaxCache::GetIBlockCacheTag($arParams['IBLOCK_ID'])]], ['GLOBAL_ACTIVE' => 'Y', '<=LEFT_BORDER' => $arSection['LEFT_MARGIN'], '>=RIGHT_BORDER' => $arSection['RIGHT_MARGIN'], 'DEPTH_LEVEL' => 1, 'IBLOCK_ID' => $arParams['IBLOCK_ID']], false, ['ID', 'IBLOCK_ID', 'NAME', 'UF_TABLE_SIZES']);
    ...
    if($tmpTableSizes) {
        $rsTypes = CUserFieldEnum::GetList([], ['ID' => $tmpTableSizes]);
...
if(!empty($arResult['DISPLAY_PROPERTIES'])) {
    ...
    foreach ($arResult['DISPLAY_PROPERTIES'] as $propKey => $arDispProp) {
        if(!in_array($arDispProp['CODE'], ['SERVICES', 'HIT', 'RECOMMEND', 'NEW', 'STOCK', 'VIDEO', 'VIDEO_YOUTUBE', 'POPUP_VIDEO', 'CML2_ARTICLE'])) {
