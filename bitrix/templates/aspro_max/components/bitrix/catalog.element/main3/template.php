...
$addParams = [];
if ($bComplect) {
	$addParams = ["DISPLAY_WISH_BUTTONS" => "N"];
}
...
if ($showCustomOffer && isset($arResult['OFFERS'][$arResult['OFFERS_SELECTED']])) {
	...
	TSolution\LinkableProperty::resolve($arResult['OFFER_PROP'], $arCurrentSKU['IBLOCK_ID'], $arResult['IBLOCK_SECTION_ID']);
} else {
	$article = $arResult["CML2_ARTICLE"];
}
...
if ($arResult["OFFERS"]) {
	...
	if (($arParams["SHOW_MEASURE"] == "Y") && ($arResult["CATALOG_MEASURE"])) {
		$arMeasure  = CCatalogMeasure::getList([], ["ID" => $arResult["CATALOG_MEASURE"]], false, false, [])->GetNext();
				...
				<? foreach ($arResult["OFFERS"] as $key => $arSKU): ?>
					...
					<div class="table-view__item item bordered box-shadow main_item_wrapper <?= ($useStores ? "table-view__item--has-stores" : ""); ?> js-notice-block">
						...
						if (isset($arFirstSkuPicture["ID"])) {
							$arFirstSkuPicture = \CFile::ResizeImageGet($arFirstSkuPicture["ID"], [
								"width"  => 60,
								"height" => 60
								...
								>
									...
									<? if ($arSKU['PROPERTIES']['ARTICLE']['VALUE']): ?>
										<div class="font_sxs muted article">
											<span class="name"><?= Loc::getMessage('ARTICLE_COMPACT'); ?></span>
											<span class="value"><?= $arSKU['PROPERTIES']['ARTICLE']['VALUE']; ?></span>
										</div>
									<? endif; ?>
								</div>
															...
															<? } else {
																if (is_array($arSKU["PROPERTIES"][$arProp["CODE"]]["VALUE"])) {
																	...
																	if ($arSKU["PROPERTIES"][$arProp["CODE"]]["USER_TYPE"] == "directory" && isset($arSKU["PROPERTIES"][$arProp["CODE"]]["USER_TYPE_SETTINGS"]["TABLE_NAME"])) {
																		$rsData = \Bitrix\Highloadblock\HighloadBlockTable::getList(['filter' => ['=TABLE_NAME' => $arSKU["PROPERTIES"][$arProp["CODE"]]["USER_TYPE_SETTINGS"]["TABLE_NAME"]]]);
						...
						<div class="offer-stores"
						     style="display: none;"
						>
							<? $APPLICATION->IncludeComponent("bitrix:catalog.store.amount", "main", [
								"PER_PAGE"                       => "10",
								"USE_STORE_PHONE"                => $arParams["USE_STORE_PHONE"],
								"SCHEDULE"                       => $arParams["SCHEDULE"],
								"USE_MIN_AMOUNT"                 => $arParams["USE_MIN_AMOUNT"],
...
<div class="product-info-wrapper">
	...
	>
		...
		<meta itemprop="category"
		      content="<?= $arResult['CATEGORY_PATH'] ?>"
		/>
											...
											if ($arParams['REVIEWS_VIEW'] == 'EXTENDED'):?>
												<div class="blog-info__rating--top-info pointer">
													...
													>
														...
														/>
														<div class="ratings">
															<? $message = $arResult['PROPERTIES']['EXTENDED_REVIEWS_COUNT']['VALUE'] ? GetMessage('VOTES_RESULT', ['#VALUE#' => $arResult['PROPERTIES']['EXTENDED_REVIEWS_RAITING']['VALUE']]) : GetMessage('VOTES_RESULT_NONE') ?>
								...
								<? if ($isArticle): ?>
									<div class="product-info-headnote__article">
										<div class="article muted font_xs"
										     ...
										     <? if (!strlen($article["VALUE"])){ ?>id="<? echo $arItemIDs["ALL_ITEM_IDS"]['DISPLAY_PROP_ARTICLE_DIV'] ?>"
										     style="display: none;"<? } ?>>
										...
										>
											...
											<? if (!$arResult["BRAND_ITEM"]["IMAGE"]): ?>
												<a href="<?= $arResult["BRAND_ITEM"]["DETAIL_PAGE_URL"] ?>"
												   class="brand__link dark_link"
												><?= $arResult["BRAND_ITEM"]["NAME"] ?></a>
											<? else: ?>
									...
									<div class="sku_props inner_content js_offers__<?= $arResult['ID']; ?>_detail load-offer-js">
										<? if (!empty($arResult['OFFERS_PROP'])) { ?>
											<div class="bx_catalog_item_scu wrapper_sku sku_in_detail"
											     ...
											     data-section_id="<?= $arResult['IBLOCK_SECTION_ID']; ?>"
											>
									...
									<? if ($bComplect): ?>
										<div class="complect_prices_block">
											...
											<div class="buy_complect_wrap hidden">
												...
												><span><?= \Bitrix\Main\Config\Option::get("aspro.max", "EXPRESSION_ADDTOBASKET_BUTTON_DEFAULT", GetMessage("EXPRESSION_ADDTOBASKET_BUTTON_DEFAULT")); ?></span></span>
											</div>
											<span class="btn btn-default btn-lg type_block has-ripple choise btn-wide"
											      data-block=".js-scroll-complect"
											><span><?= Loc::getMessage("COMPLECT_BUTTON") ?></span></span>
										</div>
									<? else: ?>
										<? $frame = $this->createFrame()->begin(); ?>
												...
												<? if ($arParams["SHOW_CHEAPER_FORM"] == "Y"): ?>
													<div class="cheaper_form muted777 font_sxs">
														...
														<span class="animate-load dotted"
														      ...
														      data-name="cheaper"
														      data-autoload-product_name="<?= CMax::formatJsName($arCurrentSKU ? $arCurrentSKU['NAME'] : $arResult['NAME']); ?>"
														      data-autoload-product_id="<?= $arCurrentSKU ? $arCurrentSKU['ID'] : $arResult['ID']; ?>"
														><?= ($arParams["CHEAPER_FORM_NAME"] ? $arParams["CHEAPER_FORM_NAME"] : Loc::getMessage("CHEAPER")); ?></span>
													</div>
												<? endif; ?>
											</div>

											<div class="quantity_block_wrapper">
												<div style="color: <?= explode('_', $arResult['PROPERTIES']['STATUS']['VALUE_XML_ID'])[1] ?>;"><?= $arResult['PROPERTIES']['STATUS']['VALUE'] ?></div>
												<pre>
<?
//print_r($arResult['PROPERTIES']['STATUS']);
?>
</pre>
											...
											<? if (!$arResult["OFFERS"]): ?>
												...
												<div class="counter_wrapp big clearfix">
													...
													<div id="<? echo $arItemIDs["ALL_ITEM_IDS"]['BASKET_ACTIONS']; ?>"
													     class="button_block <?= ($arAddToBasketData["ACTION"] === "OUT_OF_PRODUCTION" || ($arAddToBasketData["ACTION"] == "ORDER" /*&& !$arResult["CAN_BUY"]*/) || !$arAddToBasketData["CAN_BUY"] || !$arAddToBasketData["OPTIONS"]["USE_PRODUCT_QUANTITY_DETAIL"] || ($arAddToBasketData["ACTION"] == "SUBSCRIBE" && $arResult["CATALOG_SUBSCRIBE"] == "Y") ? "wide" : ""); ?>"
													>
														<!--noindex-->
												...
												<div class="offer_buy_block buys_wrapp has_offer_prop">
													<div class="counter_wrapp big clearfix">
														...
														<div id="<?= $arItemIDs["ALL_ITEM_IDS"]['BASKET_ACTIONS']; ?>"
														     class="button_block <?= ($arAddToBasketData["ACTION"] === "OUT_OF_PRODUCTION" || $arAddToBasketData["ACTION"] == "ORDER" || !$arAddToBasketData["CAN_BUY"] || !$arAddToBasketData["OPTIONS"]["USE_PRODUCT_QUANTITY_DETAIL"] || $arAddToBasketData["ACTION"] == "SUBSCRIBE" ? "wide" : ""); ?>"
														>
															<!--noindex-->
															<?= $arAddToBasketData["HTML"] ?>
															<!--/noindex-->
													...
													<? if (TSolution\Product\Price::isRangePriceMode($arCurrentSKU)): ?>
														<? $arOnlyItemJSParams = [
								...
								<div class="text-form muted ncolor">
									<div class="price_txt muted777 font_sxs ">
										...
										<div class="text-form-info">
											<span><span class="animate-load dotted"
											            data-event="jqm"
											            data-param-form_id="SEND_GIFT"
											            data-name="send_gift"
							...
							<?/* if ($arResult['HELP_TEXT']): */?><!--
								<div class="text-form">
									<div class="price_txt muted777 font_sxs muted ncolor">
										<?php /*= CMax::showIconSvg("info_big pull-left", SITE_TEMPLATE_PATH . '/images/svg/catalog/info_big.svg', '', '', true, false); */?>
										<div class="text-form-info">
											<?/* if (!$arResult['HELP_TEXT_FILE']): */?>
												<?php /*= $arResult['HELP_TEXT']; */?>
											<?/* else: */?>
												<?/* $APPLICATION->IncludeComponent(
													"bitrix:main.include",
													"",
										...
										<div class="brand-detail-info__preview">
											...
											<? if ($arResult['SECTION']): ?>
												<div class="link font_xs">
													<a href="<?= $arResult['BRAND_ITEM']['CATALOG_PAGE_URL'] ?>"
													   target="_blank"
													><?= GetMessage("ITEMS_BY_SECTION") ?></a>
												</div>
											<? endif; ?>
											<div class="link font_xs">
												<a href="<?= $arResult['BRAND_ITEM']["DETAIL_PAGE_URL"]; ?>"
												   target="_blank"
												><?= GetMessage("ITEMS_BY_BRAND", ["#BRAND#" => $arResult['BRAND_ITEM']["NAME"]]) ?></a>
											</div>
										</div>
									</div>
								</div>
		...
		<? if ($arResult['OFFERS']): ?>
			...
			>
                ...
                <? foreach ($arResult['OFFERS'] as $arOffer): ?>
	                ...
	                >
                        <meta itemprop="sku"
                              content="<?= implode('/', $currentOffersList) ?>"
                        />
	...
	<div class="ordered-block">
		<div class="additional-gallery <?= ($arResult['OFFERS'] && 'TYPE_1' === $arParams['TYPE_SKU'] && !$arResult['ADDITIONAL_GALLERY'][$arCurrentSKU['ID']] ? ' hidden' : '') ?>">
				...
				<div class="flexbox flexbox--row">
					...
					<div class="switch-item-block__icons-wrapper">
						<span class="switch-item-block__icons<?= (!$bShowSmallGallery ? ' active' : ''); ?> switch-item-block__icons--big"
						      title="<?= Loc::getMessage("BIG_GALLERY"); ?>"
						><?= CMax::showIconSvg("gallery", SITE_TEMPLATE_PATH . "/images/svg/gallery_alone.svg", "", "colored_theme_hover_bg-el-svg", true, false); ?></span>
						<span class="switch-item-block__icons<?= ($bShowSmallGallery ? ' active' : ''); ?> switch-item-block__icons--small"
						      title="<?= Loc::getMessage("SMALL_GALLERY"); ?>"
						><?= CMax::showIconSvg("gallery", SITE_TEMPLATE_PATH . "/images/svg/gallery_list.svg", "", "colored_theme_hover_bg-el-svg", true, false); ?></span>
					</div>
				</div>
			</div>
			...
			<? \Aspro\Functions\CAsproMax::showBlockHtml([
				'FILE'   => 'images/detail_big_gallery.php',
				'PARAMS' => [
					'CONFIG' => [
						'IS_ACTIVE' => !$bShowSmallGallery,
					],
					'ITEMS'  => $additionalGallery,
				]
			]); ?>
