...
if($arResult){
    ...
    if($bMenuIblock) {
        ...
        foreach($arResult as $itemKey => $item) {
            if( isset( $item['PARAMS']['FROM_IBLOCK'] ) && $item['PARAMS']['FROM_IBLOCK'] ) {
                if( isset( $item['PARAMS']['IBLOCK_ID'] ) && $item['PARAMS']['IBLOCK_ID'] ) {
                    if (!isset($arMenuIblocks[$item['PARAMS']['IBLOCK_ID']])) {
                        $arMenuIblocks[$item['PARAMS']['IBLOCK_ID']] = [];
                    }

                    $arMenuIblocks[$item['PARAMS']['IBLOCK_ID']][] = $item;
        ...
        if($arMenuIblocks) {
            foreach($arMenuIblocks as $catalog_id => $arChilds) {
                if($catalog_id){
                    ...
                    if (isset($arItem['PARAMS']['BRANDS']) && $arItem['PARAMS']['BRANDS']) {
                        foreach ($arItem['PARAMS']['BRANDS'] as $brandKey => $brandID) {
                            if ($arBrands[$brandID]) {
                                ...
                                unset($arResult[$key]['PARAMS']['BRANDS'][$brandKey]);
                            }
                        }
                    }
                }
