...
<?if($arResult['ITEMS']):?>
    ...
    <div class="item-views list list-type-block wide_img list-news1 <?= $servicesMode ? 'services-mode' : ''; ?> <?= $arParams['IMAGE_POSITION'] ? 'image_'.$arParams['IMAGE_POSITION'] : ''; ?><?= $arParams['LINKED_MODE'] == 'Y' ? ' compact-view' : ''; ?> <?= $arParams['LINKED_MODE'] != 'Y' && $arParams['SMALL_IMAGE_MODE'] ? ' small-image-view' : ''; ?> <?=$templateName; ?>-template">
        ...
        if($bHasSection) {
            ...
            $arSectionButtons = CIBlock::GetPanelButtons($arResult['SECTIONS'][$arParams['PARENT_SECTION']]['IBLOCK_ID'], 0, $arResult['SECTIONS'][$arParams['PARENT_SECTION']]['ID'], ['SESSID' => false, 'CATALOG' => true]);
            ...
            $this->AddDeleteAction($arResult['SECTIONS'][$arParams['PARENT_SECTION']]['ID'], $arSectionButtons['edit']['delete_section']['ACTION_URL'], CIBlock::GetArrayByID($arResult['SECTIONS'][$arParams['PARENT_SECTION']]['IBLOCK_ID'], 'SECTION_DELETE'), ['CONFIRM' => GetMessage('CT_BNL_ELEMENT_DELETE_CONFIRM')]);
            ?>
        ...
        <?if($bSlider):?>
            ...
            <div class="swiper-nav-offset slider-outer">
                <div class="items row margin0 <?= $bSliderWait ? ' owl-carousel-wait loader_circle ' : ' swiper '; ?> slider-solution swipeignore" data-plugin-options='<?=Bitrix\Main\Web\Json::encode($arOptions); ?>'>
    ...
    <?endif; ?>
                            ...
                            <div class="body-info <?=$bShowSectionName ? 'with-section' : ''; ?> <?=($bDetailLink) ? 'has-link' : ''; ?>">
                                            ...
                                            <?foreach($arItem['MIDDLE_PROPS'] as $PCODE => $arProperty):?>
                                                <div class="middle_prop">
                                                    ...
                                                    <div class="value font_sm darken">
                                                        ...
                                                        <?elseif($PCODE == 'PHONE'):?>
                                                            <a href="tel:<?=str_replace([' ', ',', '-', '(', ')'], '', $arProperty['VALUE']); ?>" class="dark-color"><?=$arProperty['VALUE']; ?></a>
                                                        <?else:?>
                                                            <?=$arProperty['VALUE']; ?>
                                                        <?endif; ?>
                                    ...
                                    <?if(isset($arItem['SHOW_BUY_BUTTON']) && $arItem['SHOW_BUY_BUTTON']):?>
                                        ...
                                        ?>
                                                <?if(($arAddToBasketData['OPTIONS']['USE_PRODUCT_QUANTITY_LIST'] && $arAddToBasketData['ACTION'] == 'ADD') && $arAddToBasketData['CAN_BUY']):?>
                                                    <?=CAsproMax::showItemCounter($arAddToBasketData, $arItem['ID'], $arItemIDs, $arParams, '', '', true); ?>
                                                <?endif; ?>
                                ...
                                <?if($bShowDopBlock && $arParams['SALE_MODE'] == 'Y'):?>
                                    <div class="info-sticker-block static-block">
                                        ...
                                        <?endif; ?>
                                        <?if($bDiscountCounter):?>
                                            <?CAsproMax::showDiscountCounter(0, $arItem, [], [], '', 'compact'); ?>
                                        <?endif; ?>
                                    </div>
                                <?endif; ?>
                            </div>
            ...
            <?endforeach; ?>
        <?if(!$isAjax):?>
                <?if($bSlider || ($arParams['SHOW_TITLE'] == 'Y' && $arParams['TITLE'])):?>
                </div>
                <?endif;?>
            </div>
        <?if($bSlider):?>
                ...
                TSolution\Functions::showBlockHtml([
                    ...
                    'PARAMS' => [
                        'CLASSES' => 'swiper-pagination--small visible-xs',
