        ...
        <div class="item-views reviews<?= $bCompact ? ' compact' : ''; ?><?= $bMoreItem ? ' more-item' : ''; ?> <?= $arParams['LINKED_MODE'] == 'Y' ? ' linked ' : ''; ?> <?= $notSlider ? ' list-mode ' : ''; ?>" itemscope itemtype="https://schema.org/Review">
            ...
            <?else:?>
                ...
                <div class="relative swiper-nav-offset">
                    <div class="swiper slider-solution swipeignore wsmooth1 appear-block loading_state" data-plugin-options='<?= Bitrix\Main\Web\Json::encode($arOptions); ?>'>
            ...
            $this->AddEditAction($arItem['ID'], $arItem['EDIT_LINK'], CIBlock::GetArrayByID($arItem['IBLOCK_ID'], 'ELEMENT_EDIT'));
                    ...
                    ?>
                    <div class="swiper-slide item-wrapper col-xs-12<?= !$bOneItem ? ' bg-fill-white bordered1 box-shadow1' : ''; ?>">
                        ...
                        <?php	$answer = (isset($arItem['FIELDS']['DETAIL_TEXT']) && strlen($arItem['FIELDS']['DETAIL_TEXT']) ? $arItem['FIELDS']['DETAIL_TEXT'] : '');
                    if($notSlider && $answer):// isset($arItem['DISPLAY_PROPERTIES']['STAFF'])):?>
                            <?php
                    // preview image response

                    $arStaff = (isset($arItem['DISPLAY_PROPERTIES']['STAFF']['VALUE']) && isset($arResult['STAFF'][$arItem['DISPLAY_PROPERTIES']['STAFF']['VALUE']]) ? $arResult['STAFF'][$arItem['DISPLAY_PROPERTIES']['STAFF']['VALUE']] : []);
                        $bImageResp = strlen($arStaff['PREVIEW_PICTURE']);

                        $arImageResp = ($bImageResp ? CFile::ResizeImageGet($arStaff['PREVIEW_PICTURE'], ['width' => 40, 'height' => 40], BX_RESIZE_IMAGE_EXACT, true) : []);
