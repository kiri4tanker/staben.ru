...
<div class="catalog-compare swipeignore">
    ...
    <?if ($isAjax){?>
    <script>
        arAsproCounters.COMPARE.COUNT = '<?=count($arCompareIDs);?>';
    </script>
    <?}?>
					...
					<?if($arResult["SHOW_FIELDS"]):?>
						<div class="catalog-compare__items catalog_block items block flexbox flexbox--row owl-carousel owl-theme owl-bg-nav visible-nav" data-plugin-options='{"nav": true, "autoplay" : false, "dots": false, "autoplayTimeout" : "3000", "smartSpeed":500, "responsiveClass": true, "withSlide": "catalog-compare__props-slider", "rewind": true, "responsive":{"0":{"items": 2},"768":{"items": 3},"992":{"items": 4},"1200":{"items": 5}}}'>
							<?foreach($arSection["ITEMS"] as &$arElement){?>
								<?
								unset($arElement['IPROPERTY_VALUES']);
								$totalCount = CMax::GetTotalCount($arElement, $arParams);
								$bComplect = $arElement["PROPERTIES"]["PRODUCT_SET"]["VALUE"] === "Y";

                                // price_matrix greater than 1
                                $arElement = array_merge($arElement, TSolution\Product\Price::resolveWhenEmptyPriceMatrix(result: $arResult, element: $arElement, params: $arParams));
                                if (!$arElement['CAN_BUY'] && $arElement['MIN_PRICE']) {
                                    $arElement['CAN_BUY'] = $arElement['MIN_PRICE']['CAN_BUY'];

                                    if(function_exists('CatalogGetPriceTableEx') && (isset($arElement['PRICE_MATRIX'])) && !$arElement['PRICE_MATRIX']) {
                                        $arElement['PRICE_MATRIX'] = CatalogGetPriceTableEx($arElement['ID'], 0, array_column($arResult['PRICES'], 'ID'), 'Y', $arResult['CONVERT_CURRENCY']);
                                        if ($rangeStart = (int) current(array_column($arElement['PRICE_MATRIX']['ROWS'], 'QUANTITY_FROM'))) {
                                            $arParams['EXACT_QUANTITY'] = $rangeStart;
                                        }
                                    }
                                }
                                //

								$arElement['NAME'] = (isset($arElement["OFFER_FIELDS"]["NAME"]) ? $arElement["OFFER_FIELDS"]["NAME"] : $arElement["NAME"]);
								...
								<div class="item item-parent item_block mobile">
									<div class="catalog_item_wrapp js-notice-block catalog_item item_wrap main_item_wrapper<?=($arElement["OFFERS"] ? 'has-sku' : '')?>" id="<?=$this->GetEditAreaId($arElement['ID']);?>">
										<div class="inner_wrap TYPE_2">
											<span onclick="CatalogCompareObj.MakeAjaxAction('<?=CUtil::JSEscape($arElement['~DELETE_URL'])?>', 'Y');" class="remove colored_theme_hover_text js-item-action1 added in" title="<?=GetMessage("CATALOG_REMOVE_PRODUCT")?>" data-action="compare" data-item="<?=$arElement['ID']?>" data-iblock="<?=$arElement['IBLOCK_ID']?>">
												<?=CMax::showIconSvg("remove_item", SITE_TEMPLATE_PATH.'/images/svg/catalog/cancelfilter.svg', '', '', true, false);?>
											</span>
											...
											<?$addParams = array();
												if($bComplect){
													...
													$arElement["SHOW_FROM_LANG"] = "Y";
												}
												?>

												<?\Aspro\Functions\CAsproMaxItem::showDelayCompareBtn(array_merge($arParams, $addParams), $arElement, $arAddToBasketData, $totalCount, $bUseSkuProps, 'block', false, false, '_small');?>

												<?\Aspro\Functions\CAsproMaxItem::showImg($arParams, $arElement, false);?>
											</div>
											<div class="item_info">
												<div class="item-title ">
													<a href="<?=$arElement["DETAIL_PAGE_URL"]?>" class="dark_link option-font-bold font_sm js-notice-block__title"><span><?=$arElement['NAME'];?></span></a>
												</div>
