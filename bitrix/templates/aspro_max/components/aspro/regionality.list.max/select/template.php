		...
		<div class="region_wrapper">
			<div class="io_wrapper">
				...
				<?if (
					...
					<div class="js_city_chooser dark-color list" data-param-href="<?=$arResult['URI'];?>" data-param-form_id="city_chooser">
					...
					<div class="wrap">
						<?foreach($arResult['REGIONS'] as $id => $arItem):?>
							<div class="more_item <?=($id == $arResult['CURRENT_REGION']['ID'] ? 'current' : '');?>">
								<span data-region_id="<?=$arItem['ID']?>" data-prefix="<?=$arItem['PREFIX_URL'];?>" data-href="<?=urldecode($arItem['URL'])?>"><?=$arItem['NAME'];?></span>
							</div>
						<?endforeach;?>
					</div>
				</div>
